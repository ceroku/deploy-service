...
require_relative './git_adapter.rb'
...
module Grack
  ...
  class App
    ...
    ROUTES = [
      [%r'/([a-z0-9\-]+\.git)/(git-(?:upload|receive)-pack)$',        'POST', :handle_pack],
      [%r'/([a-z0-9\-]+\.git)/info/refs$',                            'GET',  :info_refs],
      [%r'/([a-z0-9\-]+\.git)/(HEAD)$',                               'GET',  :text_file],
      [%r'/([a-z0-9\-]+\.git)/(objects/info/alternates)$',            'GET',  :text_file],
      [%r'/([a-z0-9\-]+\.git)/(objects/info/http-alternates)$',       'GET',  :text_file],
      [%r'/([a-z0-9\-]+\.git)/(objects/info/packs)$',                 'GET',  :info_packs],
      [%r'/([a-z0-9\-]+\.git)/(objects/info/[^/]+)$',                 'GET',  :text_file],
      [%r'/([a-z0-9\-]+\.git)/(objects/[0-9a-f]{2}/[0-9a-f]{38})$',   'GET',  :loose_object],
      [%r'/([a-z0-9\-]+\.git)/(objects/pack/pack-[0-9a-f]{40}\.pack)$', 'GET', :pack_file],
      [%r'/([a-z0-9\-]+\.git)/(objects/pack/pack-[0-9a-f]{40}\.idx)$', 'GET', :idx_file],
    ]

    ##
    # A list of supported authorization keys.
    AUTHORIZATION_KEYS = [
      'HTTP_AUTHORIZATION',
    ...
    def route
      ...
      ROUTES.each do |path_matcher, verb, handler|
        path_info.match(path_matcher) do |match|
          ...
          return invalid_path if bad_uri?(@repository_uri)

          # Auth related
          return unauthorized unless auth_provided?
          return bad_request unless auth_basic?
          return unauthorized unless auth_valid?

          env['REMOTE_USER'] = auth_credentials.first
    ...
    PLAIN_AND_AUTH_TYPE = {
      ...
      'WWW-Authenticate' => 'Basic realm="Concerto"'
    }
    ...
    def not_found
      [404, PLAIN_TYPE, ["\n!   No such app as " + @repository_uri.split('.').first + ".\n"]]
    end
...
! Invalid path.
! Syntax is: http://localhost:9000/<app>.git where <app> is your app's name.
...
!   See documentation for details: https://devcenter.heroku.com/articles/git#http-git-authentication
      ...
      EOF
      [401, PLAIN_AND_AUTH_TYPE, [message]]
    end
    ...
    def auth_provided?
      !authorization_key.nil? && !@env[authorization_key].nil?
    end

    def auth_basic?
      auth_parts.first && auth_parts.first.downcase == 'basic'
    end

    def auth_valid?
      auth_credentials.first == 'admin' && auth_credentials.last == 'admin'
    end
    ...
    def auth_parts
      @auth_parts ||= @env[authorization_key].split(' ', 2)
    end

    def auth_credentials
      @credentials ||= auth_parts.last.unpack("m*").first.split(/:/, 2)
    end
